...
import ChooseFilters from "./FilterandAddition/Choose/ChooseFilters";
import ChooseAddition from "./FilterandAddition/Choose/ChooseAddition";
...
const Choose = () => {
  ...
  const [chooseAddition, setChooseAddition] = useState(false);
  ...
  const getChoose = () => {
    axios
      ...
      .catch((err) => {
        console.error("Error loading choose items", err);
  ...
  const deleteChoose = () => {
    ...
    axios
      .post(`${apiBaseUrl}/choose/multiDelete`, { ids: allIds })
      .then((res) => res.data) // Get the ApiResponse object
      .then((apiResponse) => {
        if (apiResponse.success) {
          toast.success(apiResponse.message || "Items deleted successfully!");
          getChoose();
          setAllIds([]);
        } else {
  ...
  const changeStatus = () => {
    ...
    axios
      .post(`${apiBaseUrl}/choose/changeStatus`, { ids: allIds })
      .then((res) => res.data) // Get the ApiResponse object
      .then((apiResponse) => {
        if (apiResponse.success) {
    ...
    <div className="mx-auto px-4 py-6 max-w-full">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">
            Why Choosing US Listing
          </h1>
          <p className="text-sm text-blue-600">Dashboard / Why Choose Us</p>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-4 md:p-6 rounded-lg shadow mb-6 space-y-4 md:space-y-0">
        ...
        <div className="flex flex-wrap gap-2 md:gap-3 w-full md:w-auto">
          ...
          <button
            onClick={() => setChooseFilter((prev) => !prev)}
          ...
          <button
            onClick={() => navigate("/chooseaddition")}
      ...
      <div className="mt-4 mb-4">
        {chooseFilter && (
          <ChooseFilters onClose={() => setChooseFilter(false)} />
        )}
      </div>
      {/* <div className="mt-4 mb-4">
        {chooseAddition && (
          <ChooseAddition onClose={() => setChooseAddition(false)} refreshChoose={getChoose} />
        )}
      </div> */}
            ...
            <tbody className="divide-y divide-gray-200">
              {data.map((item, index) => (
                <tr key={index} className="hover:bg-gray-50 transition">
                  ...
                  <td className="px-6 py-4">
                    <img
                      src={`${apiBaseUrl.replace("/admin", "")}${item.chooseImageUrl}`}
                  ...
                  <td className="px-6 py-4 text-center">
                    <span
                      className={`px-4 py-1 rounded-full text-xs font-bold ${item.chooseStatus
                        ? "bg-green-100 text-green-800"
                        : "bg-red-100 text-red-800"
                        }`}
                    >
                      {item.chooseStatus ? "Active" : "Inactive"}
                    </span>
                  </td>
